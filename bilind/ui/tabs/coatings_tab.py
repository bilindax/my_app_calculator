...
class CoatingsTab(BaseTab):
    ...
    def _ledger_is_autocalc(self, key: str) -> bool:
        """Heuristic: detect lists generated by Auto-Calc (SSOT)."""
        items = getattr(self.app.project, f"{key}_items", []) or []
        for it in items:
            desc = it.get('desc') if isinstance(it, dict) else getattr(it, 'description', '')
            desc = str(desc or '')
            # Auto-calc uses simple descriptions without (Net ...) suffix.
            if (desc.startswith('Walls: ') or desc.startswith('Ceiling: ')) and '(Net ' not in desc and desc.startswith('Deduction:') is False:
                return True
        return False
    ...
    def add_ceilings(self, key):
        ...
        for rc in room_calcs:
            if key == 'paint':
                area = float(rc.paint_ceiling or 0.0)
                desc = f"Ceiling: {rc.room_name} (Net Paint)"
            else:
                area = float(rc.plaster_ceiling or 0.0)
                desc = f"Ceiling: {rc.room_name}"
            if area > 0:
    ...
    def add_walls(self, key):
        # Add room walls from SSOT (UnifiedCalculator)
        if not self.app.project.rooms:
            messagebox.showwarning("No Rooms", "Pick rooms first.")
            return
        ...
        for rc in room_calcs:
            if key == 'paint':
                area = float(rc.paint_walls or 0.0)
                desc = f"Walls: {rc.room_name} (Net Paint)"
            else:
                area = float(rc.plaster_walls or 0.0)
                desc = f"Walls: {rc.room_name} (Net Plaster)"
            if area > 0:
                items.append({'name': desc, 'area': area})
    ...
    def deduct_openings(self, key):
        # If the list is SSOT-generated, openings are already accounted for.
        if self._ledger_is_autocalc(key):
            messagebox.showinfo(
                "Already Included",
                "قائمة (Auto-Calc) محسوبة من UnifiedCalculator وتخصم الفتحات تلقائياً.\n"
                "إذا بدك تطابق الإكسل: لا تستخدم Deduct Openings هنا، استخدم Auto-Calc فقط."
            )
            return
        ...
        for o in openings:
            if isinstance(o, dict):
                name = o.get('name')
                w = float(o.get('width', 0.0) or o.get('w', 0.0) or 0.0)
                h = float(o.get('height', 0.0) or o.get('h', 0.0) or 0.0)
                area = float(o.get('area', 0.0) or (w * h) or 0.0)
            else:
                name = getattr(o, 'name', '')
                w = float(getattr(o, 'width', 0.0) or getattr(o, 'w', 0.0) or 0.0)
                h = float(getattr(o, 'height', 0.0) or getattr(o, 'h', 0.0) or 0.0)
                area = float(getattr(o, 'area', 0.0) or (w * h) or 0.0)
    ...
    def deduct_ceramic(self, key):
        # If the list is SSOT-generated, ceramic is already deducted for paint (and plaster is behind ceramic).
        if self._ledger_is_autocalc(key):
            messagebox.showinfo(
                "Already Included",
                "قائمة (Auto-Calc) محسوبة من UnifiedCalculator.\n"
                "- الدهان يخصم السيراميك تلقائياً\n"
                "- الزريقة محسوبة خلف السيراميك\n"
                "إذا بدك تطابق الإكسل: لا تستخدم Deduct Ceramic هنا، استخدم Auto-Calc فقط."
            )
            return
        ...
        for z in zones:
            if isinstance(z, dict):
                ...
                area = float(safe_zone_area(z) or 0.0)
            else:
